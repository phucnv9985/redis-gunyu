...
- 对redis限制更少
  ...
  - 兼容源和目的redis不同版本，支持从redis4.0到redis7.4，参考[测试](docs/test_zh.md#版本兼容测试)
- 数据一致性策略更加灵活，自动切换
...
此功能是解析RDB文件，然后将数据回放到正在运行的redis中，可以对RDB文件进行过滤。[RDB](docs/rdb_zh.md)


#### 解析RDB

解析RDB文件，输出到文件或标准输出。[RDB](docs/rdb_zh.md)
